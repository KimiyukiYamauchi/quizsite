// scripts/export-itf-csv.cjs
// data/itf.ts -> CSV（microCMSインポート用）
// 前提: 事前に `npx tsc data/itf.ts --module commonjs --target es2020 --outDir dist` 済み
...
// ===== 入力読み込み（dist/data/itf.js） =====
const inputJs = path.resolve(process.cwd(), "dist/itf.js");
if (!fs.existsSync(inputJs)) {
  console.error("❌ dist/itf.js が見つかりません。先に次を実行してください：");
  console.error(
    "   npx tsc data/itf.ts --module commonjs --target es2020 --outDir dist"
  );
  process.exit(1);
}
const mod = require(inputJs);
const itfQuestions = mod.itfQuestions || mod.default || mod;
if (!Array.isArray(itfQuestions)) {
  console.error(
    "❌ itfQuestions が配列ではありません。data/itf.ts のエクスポートをご確認ください。"
  );
  process.exit(1);
}
...
const outFile = path.join(outDir, "itf-import.csv");
...
console.log(
  "microCMS > 対象API(itf-questions) > インポートして追加 からアップロードしてください。"
);
console.log(
